...
def sample_data_from_paths(samples_indices, paths, m, num_classes, num_layers, s, seed=0, seed_reset_layer=42):
    ...
    samples_indices = samples_indices % groups_size     # the ranking within each class corresponding to each category
    yi = y[:, None].repeat(1, s ** (num_layers - 1))
    yi2 = y[:, None].repeat(1, s ** (num_layers - 1))
    indices = []
    for l in range(num_layers):
    ...
    return x, y, path_shallow, path_deep
...
class RandomHierarchyModel(Dataset):
    ...
    def __init__(
            ...
            testsize=-1,
            seed_reset_layer=42,
            path_shallow = torch.Tensor(),
            path_deep = torch.Tensor()
        ...
        self.x, self.targets, self.path_shallow, self.path_deep = sample_data_from_paths(
    ...
    def __getitem__(self, idx):
        ...
        x, y, path_shallow, path_deep = self.x[idx], self.targets[idx], self.path_shallow[idx], self.path_deep[idx]
...
class RandomHierarchyModel_Test(Dataset):
    """
    Implement the Random Hierarchy Model (RHM) as a PyTorch dataset.
    """
    ...
    def __init__(
        ...
        #print("self.samples_indices_test", samples_indices)
