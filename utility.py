...
def max_angle_metric(muc_mat):
    """
    max |<u_k, u_l>|, u_k is the mean of k-th class
    k \neq l
    """
    metric_list = []
...
def compute_info(model, dataloader, num_layers, device):
    ...
    for batch_idx, (inputs, targets, path_shallow, path_deep) in enumerate(dataloader):
        ...
        with torch.no_grad():
            if len(inputs.shape) != 2:
                input = inputs.view(inputs.shape[0], -1)
            output = model(input)
            feature_list = model.feas
            model.feas = []
            model.input = []
